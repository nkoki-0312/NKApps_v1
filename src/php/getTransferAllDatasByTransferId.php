...
//        "cannotGetTransfer": 対象外の取引のため、取得できない
//        "success": 成功
...
$sql = "SELECT count(*) FROM transfers WHERE uniq_id = :uniq_id";
$res = $PDO->prepare($sql);
$params = array(
  ":uniq_id" => $transferId
);
...
foreach( $res as $resFor ) {
  if( $resFor[0] == 0 ) {
    $result[0] = "notExistTransfer";
  }
}

// Check this user can get transfer datas
if( $result[0] == "" ) {
  $sql = "SELECT count(*) FROM transfers WHERE uniq_id = :uniq_id AND ( ( group_id = :group_id AND create_by = :create_by ) OR ( group_id IN ( SELECT group_id FROM group_relations WHERE user_id = :user_id ) ) )";
  $res = $PDO->prepare($sql);
  $params = array(
    ":uniq_id" => $transferId,
    ":group_id"=> "self",
...
// Get transfer datas
if( $result[0] == "" ) {
  $sql = "SELECT * FROM transfers WHERE uniq_id = :uniq_id";
  $res = $PDO->prepare($sql);
  $params = array(
    ":uniq_id" => $transferId
  );
